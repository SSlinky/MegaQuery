...
let
    // Simplified method when a direct file reference is detected.
    asFileUrl = (fileUrl as text) =>
        {
            Text.BetweenDelimiters(fileUrl, "//", "."),
            Text.BetweenDelimiters(fileUrl, "sites/", "/"),
            Text.BeforeDelimiter(fileUrl, "[")
        },

    // Longer method to parse a SharePoint directory path.
    asFolderUrl = (folderUrl as text) => let
    ...
    in
        result
in
    if Text.Contains(sharePointUrl, "[")
    then asFileUrl(sharePointUrl)
    else asFolderUrl(sharePointUrl)
